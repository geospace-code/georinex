    ...
    *,
    ...
    with opener(fn) as f:
        ...
        if fast:
            ...
            if meas is not None:
                obl = obl[np.any([np.char.find(obl, j) == 0
                                  for i, j in enumerate(meas)], 0)]
            ...
            timeIndex = np.cumsum(isv ==
                                  np.flatnonzero(np.char.rfind(usv, '>') == 0)
                                  )-1
            ...
            for i, sv in enumerate(usv):
                ...
                darr = np.atleast_2d(
                        np.genfromtxt(np.char.lstrip(f[isv == i], sv),
    ...
    data = data.assign_coords(sv=[s.replace(" ", "0")
                                  for s in data.sv.values.tolist()])
    # %% other attributes
    ...
    if "interval" in hdr.keys():
        ...
        try:
            data.attrs["interval"] = np.median(np.diff(data.time)
                                               / np.timedelta64(1, "s"))
        except TypeError:
            pass
    else:
